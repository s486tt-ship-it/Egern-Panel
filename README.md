...
本插件通过读取 HTTP 响应头中的 `subscription-userinfo` 字段来获取流量数据。目前支持以下两类机场：

1. **标准机场（原生支持）**：
   - 绝大多数使用 V2Board、SSPanel 等主流开源面板的机场。
   - 这类机场的订阅链接在直接请求时，就会在响应头中返回流量信息。

2. **非标/严格限制机场（兼容支持）**：
   - 部分出于防盗刷或节省资源考虑，限制了默认响应头输出的机场（如 mitce.net）。
   - 本插件**内置了特殊兼容机制**：当正常请求获取不到流量时，插件会自动在后台模拟 Clash Verge 等主流客户端的请求特征（追加 `&flag=clash` 参数等）进行二次重试，从而成功提取隐藏的流量数据。您可以直接填入原始订阅链接，插件会自动完成适配。

> **提示**：极少数完全不提供流量接口的纯手写面板可能无法显示数据，这种情况属于机场服务端限制，任何阅读器插件均无法突破。
